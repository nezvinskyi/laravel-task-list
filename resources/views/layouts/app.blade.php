...
<body
    class="container mx-auto mt-10 mb-10 max-w-lg bg-yellow-50 dark:bg-slate-700">
    <h1 class="mb-4 text-2xl text-gray-700 dark:text-yellow-50">@yield('title')
    </h1>
    <div class="" x-data="{ flash: true }">
        @if(session()->has('success'))
        <div x-show="flash" class="relative mb-10 rounded border border-green-700 bg-green-100 dark:bg-green-700 px-4 py-3 text-lg text-green-700 dark:text-green-200"
            role="alert">

            <strong class="font-bold">Success!</strong>
            <div class="">{{ session('success') }}</div>
            <span @click="flash = false" class="absolute top-0 bottom-0 right-0 px-4 py-3">
